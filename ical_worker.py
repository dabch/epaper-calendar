...
def collides(e1, e2):
    return not (e1["start"] >= e2["end"] or e2["start"] >= e1["end"])
...
def detect_collisions(drawables, base, max_):
    ...
    for e in drawables:
        ...
        for g in colgroups:
            for e_ in g:
                if collides(e, e_):
                    collided_with.append(g)
                    break
        
        print("{}: {}".format(e["title"], collided_with))
        if not collided_with:
            colgroups.append([e])
        elif len(collided_with) == 1:
            collided_with[0].append(e)
        else:
            colgroups = [group for group in colgroups if e not in group]
            newcolgroup = []
            for g in collided_with:
                for e_ in g:
                    if e_ not in newcolgroup:
                        newcolgroup.append(e_)
            newcolgroup.append(e)
            colgroups.append(newcolgroup)
    print("Final collision groups: {}".format(colgroups))

    # Now create the matrix for maximum size of events 
    for g in colgroups:
        matrix = []
        for e in g:
